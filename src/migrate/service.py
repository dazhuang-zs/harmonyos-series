    ...
    try:
        ...
        messages = [
            {"role": "system", "content": MIGRATE_SYSTEM_PROMPT},
            {
                "role": "user",
                "content": (
                    f"请将以下 {source_language} 代码转换为 {target_framework}：\n\n"
                    f"```{source_language.lower()}\n{android_code}\n```"
                ),
            },
        ]
        ...
        return {
            ...
            "before_after": {"before": android_code, "after": result},
        }

    except Exception as e:
        logger.error(f"Migration failed: {e}", exc_info=True)
        return {"migrated_code": "", "migration_notes": [], "before_after": {}}
...
def _extract_notes(code: str) -> list[str]:
    ...
    for line in code.split("\n"):
        if "//" in line and any(
            kw in line for kw in ["迁移", "注意", "差异", "对应", "替代"]
        ):
